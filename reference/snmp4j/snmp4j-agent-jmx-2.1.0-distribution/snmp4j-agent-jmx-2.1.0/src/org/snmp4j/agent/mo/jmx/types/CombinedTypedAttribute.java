/*_############################################################################
  _## 
  _##  SNMP4J-AgentJMX 2 - CombinedTypedAttribute.java  
  _## 
  _##  Copyright (C) 2005-2012  Frank Fock (SNMP4J.org)
  _##  
  _##  Licensed under the Apache License, Version 2.0 (the "License");
  _##  you may not use this file except in compliance with the License.
  _##  You may obtain a copy of the License at
  _##  
  _##      http://www.apache.org/licenses/LICENSE-2.0
  _##  
  _##  Unless required by applicable law or agreed to in writing, software
  _##  distributed under the License is distributed on an "AS IS" BASIS,
  _##  WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
  _##  See the License for the specific language governing permissions and
  _##  limitations under the License.
  _##  
  _##########################################################################*/
package org.snmp4j.agent.mo.jmx.types;

import javax.management.*;

public class CombinedTypedAttribute extends TypedAttribute {

  protected TypedAttribute[] combinedAttributes;

  public CombinedTypedAttribute(String name, Class<Integer> type,
                                TypedAttribute[] combinedAttributes) {
    super(name, type);
    this.combinedAttributes = combinedAttributes;
  }

  public TypedAttribute[] getAttributes() {
    return combinedAttributes;
  }

  public Object transformFromNative(Object nativeValue, ObjectName objectName) {
    Object[] values = (Object[])nativeValue;
    for (int i=0; ((i<values.length) &&
                   (i<combinedAttributes.length)); i++) {
      if (values[i] != null) {
        return values[i];
      }
    }
    return null;
  }

  public Object transformToNative(Object transformedValue,
                                  Object oldNativeValue, ObjectName objectName) {
    Object[] values = (Object[])oldNativeValue;
    Object[] nativeValues = new Object[values.length];
    for (int i=0; ((i<values.length) &&
                   (i<combinedAttributes.length)); i++) {
      if (values[i] != null) {
        nativeValues[i] = transformedValue;
      }
    }
    return nativeValues;
  }

  public boolean isNativeValueAlwaysNeeded() {
    return true;
  }

}
